...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('gallery-track')) {
        window.galleryApp = new InfiniteGallery();
    }
...
function updateContent() {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        // Handle special status translations
        if (key.startsWith('status.')) {
            const statusKey = key.split('.')[1];
            if (LIBRARY.statuses[statusKey]) {
                el.textContent = LIBRARY.statuses[statusKey][currentLang] || LIBRARY.statuses[statusKey].en;
            }
            return;
        }
        ...
        if (val) {
            ...
            } else if (el.closest('.nav-links')) { 
                setupNavAnimation(el);
            }
        }
    });

    // 2. Re-translate filter buttons dynamically
    if (window.galleryApp) {
        window.galleryApp.generateDynamicFilters();
        window.galleryApp.setupFilters();
    }

    // 3. Project Cards (Roles and Tile Subtitles)
    document.querySelectorAll('.project-card').forEach(card => {
        ...
        if (project) {
            ...
            if (roleEl && project.role) {
                roleEl.textContent = getRoleName(project.role);
            }
            ...
            if (catEl) {
                ...
                let catName = "";
                if (typeObj) {
                    catName = typeObj[currentLang] || typeObj.en;
                } else if (subcatObj) {
                    catName = subcatObj[currentLang] || subcatObj.en;
                } else if (catObj) {
                    catName = catObj[currentLang] || catObj.en;
                } else {
                    catName = project.category;
                }
    ...
    // Modal is rebuilt when opened, no need to force update here
}
...
function setupModal() {
    ...
    let currentProjectId = null;
        ...
        if (project.type !== 'collection') {
            ...
            // Localized date
            const pDate = (project.date && project.date.month && LIBRARY.dates[project.date.month])
                ? (LIBRARY.dates[project.date.month][currentLang] + " " + project.date.year)
                : (project.date && project.date.raw ? project.date.raw : "");
       ...
       `;
        ...
        related.forEach(rel => {
            ...
            const pDate = (rel.date && rel.date.month && LIBRARY.dates[rel.date.month])
                ? (LIBRARY.dates[rel.date.month][currentLang] + " " + rel.date.year)
                : (rel.date && rel.date.raw ? rel.date.raw : "");

            const catObj = LIBRARY.categories[rel.category] || LIBRARY.subcategories[rel.category];
            const catName = catObj ? (catObj[currentLang] || catObj.en) : rel.category;
      ...
      `;
        ...
        document.body.style.overflow = 'hidden';
            ...
            if (typeof window.populateTestModal === 'function') {
                window.populateTestModal(projectId);
            } else {
                console.error("Test Modal function not found!");
            }
        }
    };
...
// Event Listeners
    ...
    // Check automatically on load (with slight delay to ensure DOM and data are ready)
...
class InfiniteGallery {
    ...
    generateDynamicFilters() {
        ...
        filtersContainer.innerHTML = ''; // Clear fallback
        
        // Button All
        const allBtn = document.createElement('button');
        allBtn.className = 'filter-btn active';
        allBtn.setAttribute('data-filter', 'all');
        allBtn.textContent = LIBRARY.categories['all'][currentLang] || LIBRARY.categories['all'].en;
        filtersContainer.appendChild(allBtn);
        
        // Define Structure (Category => array of Subcategories)
        // Only output ones that have subcategories in dropups, others are flat buttons.
        const structure = {
            'fic': ['sf', 'ftr', 'ser'],
            'crp': ['cflm', 'ad', 'itw'],
            'doc': [],
        ...
        for (const [catKey, subcats] of Object.entries(structure)) {
            const catData = LIBRARY.categories[catKey];
            if (!catData) continue;
            ...
            if (subcats.length === 0) {
                ...
                btn.textContent = catData[currentLang] || catData.en;
                filtersContainer.appendChild(btn);
            } else {
                // Dropup
                const dropup = document.createElement('div');
                dropup.className = 'filter-dropup';
                
                const btn = document.createElement('button');
                btn.className = 'filter-btn';
                btn.setAttribute('data-filter', catKey);
                btn.textContent = catData[currentLang] || catData.en;
                dropup.appendChild(btn);
                
                const menu = document.createElement('div');
                menu.className = 'dropup-menu';
                
                subcats.forEach(subKey => {
                    const subData = LIBRARY.subcategories[subKey];
                    if (subData) {
                        const item = document.createElement('div');
                        item.className = 'dropup-item';
                        item.setAttribute('data-filter', subKey);
                        item.textContent = subData[currentLang] || subData.en;
                        menu.appendChild(item);
                    }
                });
                
                dropup.appendChild(menu);
                filtersContainer.appendChild(dropup);
            }
        }
        
        // Re-query the filter buttons now that they exist in DOM
        this.filterButtons = document.querySelectorAll('.filter-btn, .dropup-item');
    }
    ...
    setupFilters() {
        ...
        // Remove old listener if exists to prevent duplicates on language switch
        if (this._filterHandler) {
            filtersContainer.removeEventListener('click', this._filterHandler);
        }

        this._filterHandler = (e) => {
            const btn = e.target.closest('.filter-btn, .dropup-item');
            if (!btn) return;
            ...
            try {
                ...
                // Handle Active State
                document.querySelectorAll('.filter-btn, .dropup-item').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');

                // Optional: keep parent white
                ...
                const filtered = (filter === 'all')
                    ? this.originalData
                    : this.originalData.filter(p => p.category === filter || p.subcategory === filter);
        ...
        filtersContainer.addEventListener('click', this._filterHandler);
    }
